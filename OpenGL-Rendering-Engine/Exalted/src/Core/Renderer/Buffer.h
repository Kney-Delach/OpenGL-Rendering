 ...
 * Name			: Ori Lazar
 * Date			: 29/10/2019
 * Description	: An abstract buffer wrapper interface for graphics api buffers and their elements.
     .---.
   .'_:___".
   |__ --==|
   [  ]  :[|
...
namespace Exalted 
{
	...
	struct BufferElement
	{
		...
		uint32_t Offset;		// Element offset in layout buffer.
		bool Normalized;		// To normalize the input values?

		BufferElement() = default;

		inline BufferElement(ShaderDataType type, const std::string& name, bool normalized = false)
		...
		inline uint32_t GetComponentCount() const
		{
			...
			case ShaderDataType::Mat4:    return 4; //todo: fix bug here
