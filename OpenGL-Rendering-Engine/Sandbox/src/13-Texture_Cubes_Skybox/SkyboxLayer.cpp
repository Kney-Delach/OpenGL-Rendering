...
namespace Sandbox
{
	SkyboxLayer::SkyboxLayer()
		: Layer("Texture Cubemaps Layer", true)
	{
	...
	void SkyboxLayer::OnAttach()
	{
		...
		m_ModelMesh->SetVertexArray(Exalted::ObjLoader::Load(SUZANNE));
	...
	void SkyboxLayer::OnInactiveImGuiRender()
	{
		...
		if (ImGui::Button("Enable Scene [12] -> Texture Cubemaps - Skybox - Environment Mapping"))
			m_IsActive = true;
		ImGui::End();
	}
